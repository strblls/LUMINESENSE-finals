 ...
 * and admin signups, since id_review_queue is now generic
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare('SELECT account_type, account_id FROM id_review_queue WHERE id = ?');
    $stmt->bind_param('i', $queue_id);
    $stmt->execute();
    $row = $stmt->get_result()->fetch_assoc();
    $stmt->close();
    ...
    try {
        ...
        // this is the part that's now generic instead of faculty-only.
